...
with tab_perf:
    ...
    with col_tele:
        st.write("📞 **Tele-Marketing Performance (Attribution)**")
    ...
    with col_int:
        st.write("👨‍🏫 **Interviewer Performance (Attribution)**")
    ...
    # 🌟 1.3 Detailed Placement Audit with Staff Attribution for Commissions
    ...
    SELECT 
        ...
        ci.name AS City,
        GROUP_CONCAT(DISTINCT s_tele.name) AS Tele_Marketer,
        GROUP_CONCAT(DISTINCT s_int.name) AS Interviewer,
    ...
    LEFT JOIN HR_STAFF s_tele ON t.staff_id = s_tele.staff_id
    LEFT JOIN HR_INTERVIEW i ON c.candidate_id = i.candidate_id
    LEFT JOIN HR_STAFF s_int ON i.staff_id = s_int.staff_id
    WHERE 1=1 {get_time_filter('p.employment_date')} {c_filter_e}
...
with tab_audit:
    ...
    if task == "Overdue Invoices Audit (7 Attributes)":
        ...
        # Linked Time & Location Dimension
        ...
        try:
            ...
            if not df_ret.empty:
                st.write(f"🏙️ **Retention Performance: {selected_month} | {selected_city}**")
                fig = px.bar(df_ret, x='City', y='Retention_Rate', color='Retention_Rate', text='Retention_Rate',
                             title=f"Student Retention Analysis (%) - {selected_month}", color_continuous_scale='GnBu')
                fig.update_traces(texttemplate='%{text}%', textposition='outside')
                st.plotly_chart(fig, use_container_width=True)
                st.dataframe(df_ret, use_container_width=True)
            else:
                st.info(f"📅 No onboarding logs found for {selected_month}.")
        except Exception as e:
            st.error(f"Retention query failed: {e}")

    elif task == "Ghosted Blacklist (No Show)":
        # Audit logic: Interview Passed but missing from Onboarding_Log
        ...
        st.write(f"🔍 Found {len(df_ghost)} students who passed interview but never onboarded.")
        st.dataframe(df_ghost, use_container_width=True)

# --- Tab 3: SQL Console ---
with tab_console:
    st.subheader("💻 Cross-Table Penetration Console")
    user_sql = st.text_area("Execute Raw SQL Query:", value="SELECT * FROM CANDIDATE LIMIT 5;")
